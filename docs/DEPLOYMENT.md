...
| Client roadmap narrative (all users) | `FEATURE_ORCHESTRATION_ROADMAP_NARRATIVE_ENABLED` | `false` (matches SPA `orchestrationRoadmapNarrativeEnabled` static default) | `true` in sandbox after `GET /timeline` narrative field checks | `true` in prod when promoting beyond allowlist + rollout mode | Backend + Product |
