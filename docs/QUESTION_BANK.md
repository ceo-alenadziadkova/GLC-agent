...
- Узкий список вопросов на экране задаётся `**modes.pre_brief.bankIncluded`** в `intake-policy.v1.json` (плюс identity; revenue — bank id `a10`). Тонкий список id для линта/фронта дублируется в `server/src/intake/pre-brief-bank-included.json` и должен совпадать с policy (тесты в `server/src/tests/intake-brief-policy-sync.test.ts`).
