...
- **Canon `reportUse` (ADR Phase E):** в `question-bank.v1.json` у части id заданы строковые теги для `derivedFacts.reportAnchors` и промпта `intake_report_anchors` (`getQuestionBankReportUse`); примеры: `recon_company_summary`, `recon_industry_segment`, `segment_website_presence`, `marketing_value_promise`, `ux_primary_site_goal`, `strategy_pain_anchor`.
