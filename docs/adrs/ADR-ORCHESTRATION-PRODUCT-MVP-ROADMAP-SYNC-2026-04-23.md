...
This document reconciles the v8 plan with the **current repository** so backlog estimates, file paths, and **done vs remaining** work match reality. Many v8 line items were already implemented when v8 was written; do not schedule duplicate effort. For a **post–MVP (v9) delta** (what is shipped vs what is ops/verification) and a **pitfall list for long external v9 “full product” blackdrafts** (chat/PM) — wrong metric names, obsolete phases, bad E2E assumptions — see [ADR-ORCHESTRATION-POST-MVP-V9-CRITICAL-DELTA.md](./ADR-ORCHESTRATION-POST-MVP-V9-CRITICAL-DELTA.md#stale-v9-blackdraft-pitfalls) (common mistakes in long **external** v9 “full product” specs: chat, PM).
