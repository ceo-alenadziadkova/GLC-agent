...
| `pnpm test --filter orchestration` in verification boilerplate | Use **`pnpm run test:orchestration`** (frontend + server Vitest files matching the `orchestration` path filter) or **`pnpm run verify:orchestration-all`** (contract + unit + telemetry + build + bundle). |
