...
| **Consultant + client UI**     | `PortalTimelinePage.tsx`; **portal manifest wizard** `PortalRoadmapManifestWizardPage.tsx` (route `portal/audit/:id/roadmap-manifest`, flags `clientRoadmapManifestWizardEnabled` ∧ `orchestrationRoadmapUiEnabled`); Strategy Lab `StrategyLabOrchestrationPanel.tsx` / `StrategyLabOrchestratorListBody.tsx`; client nav timeline ordering via `APP_FEATURE_FLAGS.orchestrationPlanWorkspacePrimaryUxEnabled`; `ClientPostAuditCockpitSection.tsx`, `NavigationLinksSection.tsx` (flagged) |
...
- Confirm `isOrchestrationPackApiEnabled`, `isOrchestrationTimelinePrimaryUxEnabled`, `APP_FEATURE_FLAGS.clientPlanWorkspaceEnabled` (and related) are documented in `DEPLOYMENT.md` / env matrix — **no new hardcoded toggles in components**.
