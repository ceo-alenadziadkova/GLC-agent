...
- **Domain-specific `verify()`** covers all six domains via dedicated checks: `checkTech`, `checkSecurity`, `checkSeo`, `checkUx`, `checkMarketing`, `checkAutomation` (wired in `verify-kernel`).
...
- If `DecisionLayer.decide(...)` throws, pipeline remains non-fatal and applies configured fallback from `SYSTEM_DEFAULTS.decisionLayer.onErrorFallback` (current default: `accept_with_warnings`). `control_object` event includes fallback metadata (`decision_fallback_applied`, `decision_fallback_reason_code`, `decision_fallback_error`).
...
- **FactChecker:** continue expanding scenario depth and regression cases for **marketing_utp** and **automation_processes** (domain branches already implemented).
...
| Marketing / Automation fact rules | Increase deterministic scenario coverage and edge-case fixtures for existing `checkMarketing` / `checkAutomation` branches                             |
