...
| **Scope** | Reconciliation of roadmap Phases 0–5 with the **current** codebase |
...
| **`trace.causal_chain`** | Planned for cross-phase root cause | **Not in schema**; deferred (shallow linking via `assumptions.related_claim_ids` and ADR notes). |
...
- **`buildControlObject()`** produces the full **CONTROL_OBJECT v2.0-shaped** record in TypeScript (versions in code use `v2.0` strings): counts, errors, assumptions (risk + `related_claim_ids`), trace with `truth_source`, feasibility, weighted `confidence.overall`, `confidence_weights`, safety mutators, `agent_performance`, nullable `cost_control` (filled during auto-loop reruns).
...
- Rows inserted after governance publish when `EVALUATION_DATASETS_INSERT` is not `false`. Payloads sanitised before insert; `pii_sanitized` set accordingly.
...
- **Future (Phase 6+):** `trace.causal_chain` / DAG, richer claim NLP, bandits — see ADRs.
