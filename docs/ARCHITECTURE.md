...
Product questionnaire content and discovery/intake answer text: `**packages/intake-core**` JSON and registry modules. Stable HTTP `**code**` values and short API error messages stay in `**server/src/config/api-error-codes.ts**` (and related) — **do not** fold API error contract copy into the question bank.
...
| **HTTP API errors**                 | Backend contract         | `[server/src/config/api-error-codes.ts](../server/src/config/api-error-codes.ts)`, `[api-validation-copy.ts](../server/src/config/api-validation-copy.ts)`, related `*_MESSAGE` constants                         | Stable `code` + English `error` for clients                                                     |
...
| **Payload hints (server-authored)** | Backend feature          | e.g. `[upgrade-free-snapshot-copy.en.ts](../server/src/config/upgrade-free-snapshot-copy.en.ts)`                                                                                                                  | Strings embedded in persisted or API payload shapes                                             |
...
| Stable HTTP error `code` + message?                    | `**server/src/config/api-error-codes.ts`** (and related)                                                                                                                                                   |
