...
8. **§4.2 Runtime:** `pnpm run audit:ds:ci` (alias `audit:ds:runtime`) runs [`scripts/design-system-runtime-ci.mjs`](../../scripts/design-system-runtime-ci.mjs) — **0** violations; baseline and primitive-boundary grandfather files are **not** applied. **§4.1 Migration:** `pnpm run audit:ds:migration-report` regenerates [`violations-export.md`](./violations-export.md); optional `audit:ds:migration-gate` uses on-disk allowlists if you reintroduce grandfather rows during a branch. **§4.1 Drift Budget:** `pnpm run audit:ds:drift-budget` — stable drift signature count must not exceed [`scripts/design-system-drift-budget.json`](../../scripts/design-system-drift-budget.json); update with `pnpm run audit:ds:drift-budget:record` when intentionally shrinking drift — see [`roadmap-notes.md`](./roadmap-notes.md). **Patterns lock** has no allowlist. **TS color** PDF bridge: `scripts/design-system-ts-color-allowlist.txt`. Home hero cockpit: **frozen** TSX — excluded from automated DS scans (`scripts/design-system-frozen-tsx-exempt.txt`); do not batch-migrate — see [`roadmap-notes.md`](./roadmap-notes.md). Package hero card: [`marketing-surface-tokens.ts`](../../src/app/config/marketing-surface-tokens.ts) for coverage/chip/badge parity.
