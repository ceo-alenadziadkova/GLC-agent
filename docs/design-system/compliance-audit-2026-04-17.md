...
Полный индекс имён `--*` в [inventory-dump.md](./inventory-dump.md) (**365** имён в снимке `tokens.css`; пересчитать после `pnpm run audit:ds:inventory-dump`). **Обратный анализ «определён, но ни разу не встречается в src»** в данном отчёте не выполнялся отдельным скриптом — для исчерпывающего списка нужна отдельная машинная crosswalk-процедура (не входит в текущий экспорт нарушений).
