...
### HomeHeroCockpit (permanent product freeze)

- **`src/app/marketing/blocks/HomeHeroCockpit.tsx`:** **do not edit** without explicit written approval from product and design-system owners. **Not** part of DS migration batches, “safe inline purge” waves, or compliance cleanup sweeps.
- **Automation:** the file is listed in [`scripts/design-system-frozen-tsx-exempt.txt`](../../scripts/design-system-frozen-tsx-exempt.txt) and is **skipped** by `design-system-enforcement-check` (inline visual TSX), `design-system-primitive-boundary-check`, and `design-system-raw-values-check` (app scope). Loader: [`scripts/design-system-frozen-tsx-exempt.mjs`](../../scripts/design-system-frozen-tsx-exempt.mjs). Do **not** remove it from the exempt list to “fix” CI — change the product decision first.
- **Layout-only CSS:** [`src/styles/components.css`](../../src/styles/components.css) keeps `.ds-home-hero-cockpit-wrap`, stage, panel offset, cell border shell, and floating panel chrome only.
...
- **Safe inline purge (product + marketing):** low-risk `inline-visual-style` blocks were moved to `.ds-*` in `components.css` (intake brief / wizard, activity feed, score distribution + chart legend, workspace sidebar, portal mirror, question-bank studio, login anonymous hint, full-audit domain chips). **HomeHero cockpit** is **out of scope** (see § HomeHeroCockpit). **Package marketing hero** coverage card / chips use [`marketing-surface-tokens.ts`](../../src/app/config/marketing-surface-tokens.ts) + inline `style` (product parity).
