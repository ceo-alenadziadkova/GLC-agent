...
- Where **Narrative Timeline** (`?view=timeline`) is still surfaced under **`FEATURE_PLAN_NARRATIVE_TIMELINE`** / **`planNarrativeTimelineEnabled`**, it is a **co-equal Plan surface** with **Delivery Board** on unified `/plan/:id` (**same orchestration pack** as structural truth—segment switch only changes projection). Timeline stays **read / narrative framing**; Board remains the **mutation surface** for operational state (`PATCH …/plan/board/cards`). Product + ticket artefact (**Proposed**): [`ADR-DELIVERY-BOARD-REPLACES-NARRATIVE-TIMELINE-PROPOSED-V1.md`](../adrs/ADR-DELIVERY-BOARD-REPLACES-NARRATIVE-TIMELINE-PROPOSED-V1.md). Engineering SSOT (**Accepted**): [`ADR-DELIVERY-BOARD-OPERATIONAL-LAYER.md`](../adrs/ADR-DELIVERY-BOARD-OPERATIONAL-LAYER.md).
- Bare `/plan/:id` and `/portal/plan/:id` without `view`: derive default from **`planDeliveryBoardRolloutMode`** ([`defaultPortalPlanSurfaceFromRollout`](../../src/app/config/portal-plan.ts)) — **`ga`** → default **`board`**; modes below **`ga`** still resolve through **`parsePortalPlanViewParam`** / **`PortalPlanPage`** (Delivery Board tab visibility uses **`isPlanDeliveryBoardUiEnabled`** from **`shadow`…** — see [`plan-delivery-board-ui.ts`](../../src/app/config/plan-delivery-board-ui.ts)). Legacy **`?view=timeline`** deep links collapse per **`PortalPlanPage`** when Board rollout is on ([`ADR-DELIVERY-BOARD-OPERATIONAL-LAYER.md`](../adrs/ADR-DELIVERY-BOARD-OPERATIONAL-LAYER.md)).
- Narrative Timeline UI may be retired; parity signals (**`top_7d`/`top_30d`**, **`analysis_depth`**, lane mix) ship via **`GET …/plan/board`** **`timeline_parity`** and optional **`GET …/timeline`** when orchestration still fetches it ([`docs/API.md`](../API.md)). Emergency revert uses **`FEATURE_PLAN_NARRATIVE_TIMELINE`** + **`planNarrativeTimelineEnabled`** mirrors ([DEPLOYMENT.md](../DEPLOYMENT.md)).
