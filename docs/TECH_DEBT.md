...
| TD-024 | accepted_risk | P3 | Delivery Board | **Reconcile racing `PATCH`** — reliable cross-request serialization needs a **single Postgres transaction** or session-scoped client; PostgREST + `supabase-js` split calls do not hold advisory locks across requests (`ADR-DELIVERY-BOARD-OPERATIONAL-LAYER.md` Appendices **C**, **D**). | Mitigations in production: reconcile idempotency, `expected_pack_version` / idempotency on PATCH, reconcile as pure snapshots; **monitor** unusual **`409`** (`AUDITS_ORCHESTRATION_PACK_STALE_VERSION`, idempotency conflicts) shortly after **`plan_board_reconciled`** bursts; **`optional later:`** SECURITY DEFINER routine or direct pooled PG session. No transactional hardening unless incident trend. | ADR Delivery Board 2026-05; Appendix G rollout monitoring |
