...
export function AdminSnapshotQueue() {
  ...
  return (
    <AppShell
      ...
      actions={(
        <button type="button" className="glc-btn-secondary text-sm" onClick={() => void q.refetch()}>
          <ArrowsClockwise className="w-4 h-4" /> Refresh
        </button>
      )}
    >
      <div className="px-7 py-6 max-w-5xl mx-auto space-y-4">
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0">
                        ...
                        <Link to={`/audit/${audit.id}`} className="text-xs font-medium no-underline" style={{ color: 'var(--glc-blue)' }}>
                          Open audit
                        </Link>
                      </div>
                    </div>
