...
type PlanBoardBulkActionsBarProps = {
...
export function PlanBoardBulkActionsBar(props: PlanBoardBulkActionsBarProps) {
    ...
    <div className="flex flex-wrap items-center gap-2 rounded-md border border-border bg-muted/40 px-2 py-2">
      ...
      {props.columns.map((col) => (
        <Button
          ...
          disabled={props.dragLocked || props.busy}
        >
          {`Move to ${col.title}`}
        </Button>
      ))}
      ...
      <Button type="button" variant="outline" size="sm" className="h-7 text-xs" onClick={props.onApplyPriority} disabled={props.busy}>
        Apply priority
      </Button>
      <input
      ...
      <Button
        ...
        disabled={props.busy || props.bulkAssignee.trim() === ''}
      >
        Apply assignee
      </Button>
      <input
      ...
      <Button
        ...
        disabled={props.busy || props.bulkDueDate === ''}
      >
        Apply due
      </Button>
