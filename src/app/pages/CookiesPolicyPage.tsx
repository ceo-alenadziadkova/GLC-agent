...
import type { CookiesBlock, CookiesSection } from '../config/cookies-policy-page.en';
import { COOKIES_POLICY_PAGE_EN } from '../config/cookies-policy-page.en';
import { MarketingLayout } from '../marketing/MarketingLayout';
import { MarketingSection } from '../marketing/blocks/MarketingSection';

function CookiesBlockView({ block }: { block: CookiesBlock }) {
  if (block.type === 'list') {
    return (
...
function CookiesSectionView({ section }: { section: CookiesSection }) {
  return (
    <section id={section.id} className="ds-pattern-legal-doc-section" aria-labelledby={`${section.id}-heading`}>
      ...
      {section.blocks.map((block, index) => (
        <CookiesBlockView key={index} block={block} />
      ))}
    </section>
  );
}
      ...
      <MarketingSection>
        <article lang={htmlLang} className="ds-pattern-legal-doc-container">
          ...
          {sections.map(section => (
            <CookiesSectionView key={section.id} section={section} />
          ))}
