...
describe('PlanModeBar', () => {
  it('renders three mode tabs linking to the same plan path', () => {
    render(
      <MemoryRouter initialEntries={['/plan/audit-z/board']}>
        <Routes>
          <Route path="/plan/:id/board" element={<PlanModeBar />} />
        </Routes>
      </MemoryRouter>,
    );
    ...
    expect(tabs[2]).toHaveAttribute('href', expect.stringMatching(/plan\/audit-z\/board/));
  });
});
