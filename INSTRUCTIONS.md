# TESTING — INSTRUCTIONS.md (project-specific)

## Marketing Command Layer (Global Routing)

`CMO-INSTRUCTIONS.md` is the primary marketing command file for this project.
Treat it as the main CMO Director operating system for marketing diagnostics, strategy selection, agent orchestration, and execution planning.

When to route to `CMO-INSTRUCTIONS.md`:

- when the task is about marketing strategy, positioning, ICP, messaging, content, distribution, traffic, growth loops, or campaign planning;
- when a marketing decision requires trade-off analysis and prioritization;
- when outputs need awareness-stage mapping, channel strategy, or 30-day marketing action plans;
- when the team needs updated project introduction, value narrative, or strategy synchronization before launching marketing work.

How to use it:

1. Open `CMO-INSTRUCTIONS.md` first for all strategic marketing requests.
2. Follow its diagnostic protocol before generating recommendations.
3. If marketing context is outdated, update the mandatory project-introduction snapshot there first.
4. Keep QA/testing guidance in this file as the source of truth for quality workflows.
