-- Explicit audits.no_public_website flag (source of truth) alongside legacy sentinel company_url.
-- discovery_convert_session_atomic gains p_no_public_website for discovery-origin audits.

ALTER TABLE public.audits
  ADD COLUMN IF NOT EXISTS no_public_website boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.audits.no_public_website IS
  'When true, skip outbound fetches against company_url; brief/intake only. Legacy rows may rely on sentinel URL until backfilled.';

-- Dev default sentinel shipped in @glc/dev-brand-defaults (see NO_PUBLIC_WEBSITE_URL).
UPDATE public.audits
SET no_public_website = true
WHERE company_url = 'https://glc-audit.placeholder/no-public-website';

-- Replace RPC: drop old 6-arg signature so PostgREST targets a single function.
DROP FUNCTION IF EXISTS public.discovery_convert_session_atomic(
  text,
  uuid,
  text,
  text,
...
CREATE OR REPLACE FUNCTION public.discovery_convert_session_atomic(
  ...
  p_review_phases INT[],
  p_domain_keys TEXT[],
  p_no_public_website BOOLEAN DEFAULT FALSE
)
RETURNS TABLE (
  audit_id UUID,
  error_code TEXT,
...
BEGIN
  ...
  INSERT INTO audits (user_id, company_url, company_name, industry, product_mode, no_public_website)
  VALUES (p_consultant_id, p_company_url, v_company_name, v_industry, p_product_mode, p_no_public_website)
