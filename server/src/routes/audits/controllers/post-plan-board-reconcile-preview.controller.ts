...
import {
  ...
  PLAN_BOARD_RECONCILE_PREVIEW_DISABLED_MESSAGE,
} from '../../../config/api-user-messages.en.js';
import {
  isOrchestrationPackApiEnabled,
  isPlanBoardReconcileDiffPreviewEnabled,
...
import { buildPlanBoardReconcilePreviewForAudit } from '../../../services/plan-board/plan-board-reconcile-preview.service.js';
import { logger } from '../../../services/logger.js';
import { sendApiError } from '../mappers/audits-http.mapper.js';

export async function postPlanBoardReconcilePreviewController(req: AuthRequest, res: Response) {
  try {
    ...
    if (!isPlanBoardReconcileDiffPreviewEnabled()) {
      sendApiError(
        res,
        403,
    ...
    const built = await buildPlanBoardReconcilePreviewForAudit({
      auditId,
      pack: persisted.pack,
      orchestration_pack_version: persisted.orchestration_pack_version,
    });
    if (!built.ok) {
      logger.error('route.plan_board_reconcile_preview_load_failed', { auditId, error: built.error.message });
    ...
    logger.error('route.plan_board_reconcile_preview_unhandled', { error: error.message });
    sendApiError(res, 500, API_ERROR_CODES.AUDITS_FETCH_FAILED, AUDITS_FETCH_FAILED_MESSAGE);
  }
}
