...
| ADR-CLIENT-UNIFIED-ROADMAP | Timeline primary; Lab = deep-dive | **done** (MVP) | `PortalTimelinePage.tsx` (subtitle when `orchestrationTimelinePrimaryUxEnabled`; **vN→vN+1 revision story**; **sync-marker cross-lane narrative**; **Detail pack** one-click from top actions when `clientExecutionPackTimelineSurfaceEnabled`; client errors mapped via `format-execution-pack-timeline-request-error.ts` + `strategy-execution-pack-api-error-codes.ts`), `ORCHESTRATION_IA_COPY` + `ClientPostAuditCockpitSection.tsx`, `NavigationLinksSection.tsx`, `StrategyLabOrchestrationPanel.tsx` / `strategy-lab-copy.ts`, nav flags in `app-feature-flags.ts`, `app-shell-nav.test.ts` |
...
Use when merging work mapped to backlog **V1–V12** in `docs/adrs/ADR-ORCHESTRATION-AND-ROADMAP-ROLLOUT-PLAN.md` (see that ADR **Post-MVP PR ritual** for the same commit discipline):
