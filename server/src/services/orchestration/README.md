...
**Conflict synthesis monitoring (rollout):** structured logs use the `orchestration_synthesis.*` event prefix in `orchestration-synthesis.service.ts` and `orchestration-pack-synthesis-claude.ts` (e.g. `orchestration_synthesis.rollout_applied`, `orchestration_synthesis.rollout_skip`, `orchestration_synthesis.claude_failed`, `orchestration_synthesis.validation_failed` (plus `llm_tool_validation_failed` on `pipeline_events` with capped raw tool JSON)). Tune rollout with `FEATURE_ORCHESTRATION_CONFLICT_SYNTHESIS` and `FEATURE_ORCHESTRATION_CONFLICT_SYNTHESIS_ROLLOUT_PERCENT` (see `DEPLOYMENT.md` matrix). On failure, the pack stays **deterministic** (topology invariant); KPI `kpi_orchestration_synthesis_deterministic_fallback` is defined in `orchestration-telemetry-policy.ts`.
