...
export async function loadPipelineStatus(params: { auditId: string; userId: string }): Promise<PipelineStatusResult> {
  ...
  return { ok: true, payload: toPipelineStatusPayload(audit, events, reviews) };
}
