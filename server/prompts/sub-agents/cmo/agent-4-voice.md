...
- Use positioning context from upstream agents when available.
- Adapt tone to audience maturity and trust level rather than brand vanity.
- When context is conflicting, default to clarity and credibility over stylistic aggression.

## Quality bar

- `voice_principles` must be enforceable in editorial review.
- `forbidden_phrases` must include concrete anti-pattern language, not abstract warnings.
- `vocabulary_do` should favor precise words that reinforce differentiation and trust.
